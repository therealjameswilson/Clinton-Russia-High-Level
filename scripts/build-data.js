...
function nara7388808LeadTopics(lead) {
  ...
  if (lead.potentialStandaloneCandidate) topics.push("Potential standalone FRUS document");
...
function buildNara7388808YeltsinSearchRecords() {
  ...
  const searchTrail = {
    ...
    subjectDigest:
      `${summary.totalCatalogHits || 0} total hits / ${summary.screenedCandidateHits || 0} screened OCR-title candidates / ${promoted.length} promoted visible leads / ${summary.potentialStandaloneCandidates || 0} potential standalone candidates`
  };

  const records = promoted.map((lead) => {
    const topics = nara7388808LeadTopics(lead);
    const standalone = Boolean(lead.potentialStandaloneCandidate);
    ...
    const extractionStatus =
      ...
      (standalone
        ? "Potential standalone FRUS document: source PDF needs page-level review. If adopted, extract only the actual standalone document pages and append the original marker/provenance page."
    ...
    return {
      ...
      researchPlanLead: true,
      nara7388808SearchLead: true,
      naraStandaloneCandidate: standalone || undefined,
...
function compilerAction(record) {
  if (Number.isInteger(record.pageCount)) {
    return record.extractionStatus || "Read for FRUS selection; face-page classification and distribution markings still require final compiler review.";
  }
  return (
    record.extractionStatus ||
...
function buildCompilerStartHereMarkdown(records, pageTallies, compilerRiskAudit) {
  ...
  const consolidated = pageTallies.consolidated;
  ...
  const lines = [
    ...
    `- Extracted derivative PDFs: ${compilerRiskAudit.coreConversationReadiness.extractedDerivativePdfs}`,
    "",
...
function buildCompilerStartHereHtml(records, pageTallies, compilerRiskAudit) {
  const rows = buildCompilerChronologyRows(records);
  const counted = rows.filter((row) => row.status === "counted");
  const pending = rows.filter((row) => row.status !== "counted");
  const consolidated = pageTallies.consolidated;
...
function writeOutputs(records) {
  ...
  const summary = {
    ...
    sources: {
      ...
      provisionalDocumentList: "reports/provisional-document-list.html",
      provisionalDocumentListJson: "reports/provisional-document-list.json",
      provisionalDocumentListCsv: "reports/provisional-document-list.csv",
